...
print(f"     Media: {df_clean['PuntajeXY'].mean():.4f}, Std: {df_clean['PuntajeXY'].std():.4f}")
print(f"     Min: {df_clean['PuntajeXY'].min():.4f}, Max: {df_clean['PuntajeXY'].max():.4f}")
...
print(f"   ROC-AUC PuntajeXY: {auc_xy:.4f} ({'invertido' if auc_xy < 0.5 else 'normal'})")
...
fpr_ab, tpr_ab, _ = roc_curve(df['Default'], -df['PuntajeAB'] if auc_ab < 0.5 else df['PuntajeAB'])
fpr_xy, tpr_xy, _ = roc_curve(df_clean['Default'], -df_clean['PuntajeXY'] if auc_xy < 0.5 else df_clean['PuntajeXY'])

ax.plot(fpr_ab, tpr_ab, label=f'Proveedor AB (AUC={auc_ab_direction:.4f})', linewidth=2, color='#3498db')
ax.plot(fpr_xy, tpr_xy, label=f'Proveedor XY (AUC={auc_xy_direction:.4f})', linewidth=2, color='#e74c3c')
...
def analyze_by_deciles(df, score_col, target_col, label):
    ...
    if roc_auc_score(df_analysis[target_col], df_analysis[score_col]) < 0.5:
        df_analysis['decile'] = pd.qcut(df_analysis[score_col], 10, labels=False, duplicates='drop')
    ...
    decile_stats.columns = ['defaults', 'total', 'default_rate', 'mean_score', 'min_score', 'max_score']
...
# AB - stacked bar
decile_ab[['pct_good', 'pct_bad']].plot(kind='bar', stacked=True, ax=axes[0],
...
# XY - stacked bar
decile_xy[['pct_good', 'pct_bad']].plot(kind='bar', stacked=True, ax=axes[1],
                                          color=['#2ecc71', '#e74c3c'], width=0.7)
...
ax.plot(decile_ab.index, decile_ab['default_rate']*100, 'o-', linewidth=2, label='AB', color='#3498db')
ax.plot(decile_xy.index, decile_xy['default_rate']*100, 's-', linewidth=2, label='XY', color='#e74c3c')
ax.axhline(y=df['Default'].mean()*100, color='gray', linestyle='--', label='Tasa promedio')
ax.set_xlabel('Decil')
...
if len(df_clean_first) > 0 and len(df_clean_second) > 0:
    psi_xy = calculate_psi(df_clean_first['PuntajeXY'].values, df_clean_second['PuntajeXY'].values)
else:
    psi_xy = np.nan

print(f"\n   PSI PuntajeAB: {psi_ab:.4f}")
print(f"   PSI PuntajeXY: {psi_xy:.4f}" if not np.isnan(psi_xy) else "   PSI PuntajeXY: N/A (datos insuficientes)")
...
# AB
for label, val in [(f'No Default (n={len(df[df.Default==0])})', 0), 
                    f'Default (n={len(df[df.Default==1])})', 1)]:
    sns.kdeplot(df[df.Default == val]['PuntajeAB'], label=label, ax=axes[0], fill=True, alpha=0.3)
...
for label, val in [('No Default', 0), ('Default', 1)]:
    sns.kdeplot(df_clean[df_clean.Default == val]['PuntajeXY'], label=label, ax=axes[1], fill=True, alpha=0.3)
...
print(f"   AB: Buenos={mean_ab_good:.4f}, Malos={mean_ab_bad:.4f}, Separación={abs(mean_ab_good - mean_ab_bad)/std_ab:.4f}")
...
print(f"   XY: Buenos={mean_xy_good:.4f}, Malos={mean_xy_bad:.4f}, Separación={abs(mean_xy_good - mean_xy_bad)/std_xy:.4f}")
...
def calculate_lift(df, score_col, target_col, n_bins=10):
    ...
    if roc_auc_score(df_lift[target_col], df_lift[score_col]) < 0.5:
        df_lift['rank'] = pd.qcut(df_lift[score_col], n_bins, labels=range(n_bins, 0, -1), duplicates='drop')
    else:
        df_lift['rank'] = pd.qcut(df_lift[score_col], n_bins, labels=range(n_bins, 0, -1), duplicates='drop')
    
    lift = df_lift.groupby('rank').agg({
    ...
    lift['cumulative_pct'] = (lift['count'].cumsum() / lift['count'].sum() * 100).round(1)
    lift['cumulative_defaults'] = lift['count'].cumsum() / df_lift[target_col].sum() * 100
...
print(f"""
{'Métrica':<25} {'Proveedor AB':<20} {'Proveedor XY':<20}
{'-'*65}
...
PSI (Estabilidad):       {psi_ab:<20.4f} {psi_xy:.4f if not np.isnan(psi_xy) else 'N/A':<20}
